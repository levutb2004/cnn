...
def PixAdminTransform(
    ...
    # Initialize MLflow
    mlflow.set_experiment("PixAdminTransform")
    ...
    with mlflow.start_run(run_name="test"):
        ...
        mlflow.log_params({
            "train_level": params["train_level"],
            "validation_split": params["validation_split"],
            "validation_fold": params["validation_fold"],
            ...
            "small_net": params["small_net"],
            "population_target": params["population_target"],
            "sampler": params["sampler"],
            "memory_mode": params["memory_mode"],
            "train_dataset": str(train_dataset_name),
            "test_datasets": str(test_dataset_names)
        })
        
        if params["optim"] == "adamw":
            mlflow.log_param("weight_decay", params["weights_regularizer_adamw"])

        #### prepare Dataset ######################################################################### 

        dataset = MultiPatchDataset(datalocations, train_dataset_name, params["train_level"], params['memory_mode'], device, 
            params["custom_split"], params["validation_split"], params["validation_fold"], params["weights"], params["custom_sampler_weights"],
            ...
            for epoch in tnr:
                for sample in tqdm(train_loader, disable=params["silent_mode"]):
                    ...
                    if batchiter % 50 == 0: 
                        ...
                        if len(y_pred)==2:
                            mlflow.log_metrics({
                                "train_y_pred": y_pred[0].item(),
                                "train_y_var": y_pred[1].item(),
                                "train_y_gt": y_gt.item()
                            }, step=batchiter)
                        else:
                            mlflow.log_metrics({
                                "train_y_pred": y_pred.item(),
                                "train_y_gt": y_gt.item()
                            }, step=batchiter)
                    ...
                    if itercounter>=( params['logstep'] ):
                        ...
                        for key, value in log_dict.items():
                            ...
                            if isinstance(value, (int, float, np.number)):
                                ...
                                mlflow_metrics[mlflow_key] = value
                        
                        if mlflow_metrics:
                            mlflow.log_metrics(mlflow_metrics, step=batchiter)
        ...
        with torch.no_grad():
            ...
            checkpoint_path = 'checkpoints/{}{}.pth'.format('Final/Maxstepstate_', wandb.run.name)
            ...
            mlflow.pytorch.log_model(mynet, "model")
            ...
            for key, value in log_dict.items():
                if isinstance(value, torch.Tensor):
                    value = value.item()
                if isinstance(value, (int, float, np.number)):
                    mlflow_key = "final_" + key.replace('/', '_')
                    final_mlflow_metrics[mlflow_key] = value
